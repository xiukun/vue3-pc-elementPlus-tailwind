    ...
    <div class='position'>
        <el-scrollbar ref='scrollbar' wrap-class='scrollbar-wrapper'>
            <div class='layout-tags-container whitespace-nowrap'>
                <span v-for='(v,i) in tagsList' :key='v.path' :ref='el => { layoutTagsItem[i] = el }' class='border border-gray-200 px-2 py-1 mx-1 cursor-pointer rounded-md' :class='{"layout-tags-active": v.isActive}' @contextmenu.prevent='contextRightMenu(v,$event)'>
...
const rightMenu = (
    ...
    const closeOther = () =>
        store.commit('layout/removeOtherTagNav', currentRightTags);
...
const tagScroll = (store: Store<IState>) => {
    ...
    const scrollbar: Ref<{ wrap: HTMLElement; update(): void } | null> = ref(
        null
    );
        ...
        () =>
            nextTick(() => {
                ...
                nextTick(() => {
                    ...
                    if (!scrollbar.value) return;
                    const scrollLeft =
                        itemWidth - scrollbar.value.wrap.offsetWidth + 70;
                    if (scrollLeft > 0)
                        scrollbar.value.wrap.scrollLeft = scrollLeft;
                });
            })
    );
    // 确保在每次变更之前重置引用
...
export default defineComponent({
    ...
    setup() {
        ...
        const removeTagNav = (v: any) =>
            store.commit('layout/removeTagNav', {
                cPath: route.path,
                tagsList: v,
            });
