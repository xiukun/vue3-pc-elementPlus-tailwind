...
import { ILayout, IMenubarStatus, ITagsList } from '@/type/store/layout'
...
const state: ILayout = {
    ...
    token: token || '',
    showSettings: (setting && setting.showSettings) ?? false,
    setting: {
        theme: (setting && setting.theme) ?? 0,
        showTags: (setting && setting.showTags) ?? true
    },
    isLoading: false
}
const mutations = {
    ...
    removeTagNav(
        state: ILayout,
        obj: { tagsList: ITagsList; cPath: string }
    ): void {
        const index = state.tags.tagsList.findIndex(
        ...
        if (state.tags.tagsList[index].path === obj.cPath) {
            ...
            mutations.removeCachedViews(state, {
                name: obj.tagsList.name,
                index
            })
    ...
    addCachedViews(
        state: ILayout,
        obj: { name: string; noCache: boolean }
    ): void {
        if (!state.setting.showTags) return // 判断是否开启多标签页
    ...
    login(state: ILayout, token = ''): void {
        ...
        router.push(
            typeof query.from === 'string'
                ? decodeURIComponent(decodeURIComponent(query.from))
                : '/'
        )
    },
    ...
    setRoutes(state: ILayout, data: Array<RouteRecordRaw>): void {
    ...
    getUser(
        state: ILayout,
        userInfo: { name: string; role: Array<string> }
    ): void {
    ...
    // 是否显示主题设置抽屉层
    changeSetting(state: ILayout, data: boolean): void {
        state.showSettings = data
    }
}
const actions = {
    async login(
        context: ActionContext<ILayout, IState>,
        param: loginParam
    ): Promise<void> {
